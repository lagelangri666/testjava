...
//            workbook = new HSSFWorkbook(inputStream);
    ...
    protected ExcelDataVO convertRowToData(Row row) {
        ...
        if (null == maxTempStr || "".equals(maxTempStr)) {
            ...
            if (maxTempStr.length() < sizeLength) {
                int remain = sizeLength - maxTempStr.length();
                for (int i = 0; i < remain; i++) {
                    maxTempStr = maxTempStr + " ";
                }
            }
            //String doubleMaxtemp=String.valueOf(maxTempDouble);
            resultData.setMaxtemp(maxTempStr);
        }
        //获取最低气温
        cell = row.getCell(cellNum++);
        ...
        if (null == minTempStr || "".equals(minTempStr)) {
            ...
            if (minTempStr.length() < sizeLength) {
                int remain = sizeLength - minTempStr.length();
                for (int i = 0; i < remain; i++) {
            ...
            resultData.setMintemp(minTempStr);
        }
