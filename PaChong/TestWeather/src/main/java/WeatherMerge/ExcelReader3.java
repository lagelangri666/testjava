...
//            workbook = new HSSFWorkbook(inputStream);
    ...
    protected  ExcelDataVO convertRowToData(Row row) {
        ...
        String minVPStr = convertCellValueToString(cell);
        if (null == minVPStr || "".equals(minVPStr)) {
            resultData.setVp(null);
        }
        else {
            minvp = Double.valueOf(minVPStr);
            if(minvp == 3276.6){
                minvp = 0.00;
            }
        }
        //获取大型蒸发量气温
        cell = row.getCell(cellNum++);
        String maxVPStr = convertCellValueToString(cell);
        if (null == maxVPStr || "".equals(maxVPStr)) {
            resultData.setVp(null);
        } else {
            maxvp = Double.valueOf(maxVPStr);
            if(maxvp == 3276.6){
                maxvp = 0.00;
            }
        }
        ...
        if(maxvp>=minvp){
            ...
            String vp = atempD.format(maxvp);//返回的是String类型
            if (vp.length() < sizeLength) {
                int remain = sizeLength - vp.length();
                for (int i = 0; i < remain; i++) {
                    vp = vp + " ";
                }
            }
            ...
            DecimalFormat atempD= new DecimalFormat("0.00");//格式化小数
            String vp = atempD.format(minvp);//返回的是String类型
            if (vp.length() < sizeLength) {
                int remain = sizeLength - vp.length();
                for (int i = 0; i < remain; i++) {
                    vp = vp + " ";
